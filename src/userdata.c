...
void UserData_Init() {
	...
	// Default Save data
	// All 0x00 except:
	//  - 0x03: Intro cutscene ID = 1
	Uint8 default_savedata[256] = {
	...
	user_data_blocks[2] = Datablock_Create(UDATA_DBID_SAVEINFO, default_savedata+20, 16*3);	// Save profiles header // Offset is so it's a full range of all 0s
	user_data_blocks[3] = Datablock_Create(UDATA_DBID_SAVEDATA, default_savedata, 256);		// Profile 1
	user_data_blocks[4] = Datablock_Create(UDATA_DBID_SAVEDATA, default_savedata, 256);		// Profile 2
	user_data_blocks[5] = Datablock_Create(UDATA_DBID_SAVEDATA, default_savedata, 256);		// Profile 3
